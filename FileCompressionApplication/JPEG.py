...
def compressimg(image):
    ...
    # cv2.imwrite("compressed_image.jpeg",np.uint8(padded_img))
...
def JPEG(filename):
    ...
    # cv2.imwrite('uncompressed.jpeg', np.uint8(padded_img))
    ...
    res=compressimg(bitstream)

    file_extension =Path(filename).suffix
    compressfilestr=os.path.splitext(f"{filename}")[0];
    cv2.imwrite(f"{compressfilestr}_compress{file_extension}",res)
    print("JPEG Compression Done.....")
